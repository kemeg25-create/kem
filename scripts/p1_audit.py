...
backend = Path('functions/index.js').read_text()
...
for fn in ['renderShopProducts','filterProducts','filterByCategory','openProductDetail','renderProducts','renderOrders','loadOrders','renderCustomers','loadStoreSettings','saveStoreSettings','logout','logoutEmployee']:
    ...
    print('function', fn, 'defs', len(re.findall(pat, html)))

for term in ['search', 'filter', 'account', 'orderHistory', 'savedAddresses', 'loading', 'No products', 'No orders', 'No customers']:
    print('term', term, 'count', len(re.findall(re.escape(term), html, re.I)))
...
print('product.image mentions', len(re.findall(r'product\.image\b', html)))
print('product.images mentions', len(re.findall(r'product\.images\b', html)))
print('setInterval count', html.count('setInterval('), 'requestAnimationFrame count', html.count('requestAnimationFrame('))
print('external scripts', re.findall(r'<script[^>]*src=["\']([^"\']+)', html, re.I))
print('external styles', re.findall(r'<link[^>]*href=["\']([^"\']+)', html, re.I))
...
for token in ["await auth.signOut()", "revokeEmployeeSession", "requireVerifiedCustomer", "paymentMethod !== 'cod'", "getEmployeeForRequest(request", "readPublicPath('coupons')"]:
    print('P0', token, token in html or token in backend)
