...
# Remove misleading/broken metadata and make payment claims accurate.
...
# Dead payment client code/scripts are unnecessary while P0 explicitly rejects non-COD.
...
# Client storage SDK is no longer used; product uploads go through protected callable.
...
# Remove unused JWT helper left behind after Google auth moved to Firebase credential exchange.
remove_fn('parseJwt')

# Shareable product URLs + accurate dynamic metadata without creating a new page architecture.
...
# Add URL option handling to current product open/close functions.
old_sig='function openProductDetail(productId) {'
h=h.replace(old_sig,'function openProductDetail(productId, options = {}) {',1)
...
# Current close is a compact single-line implementation.
...
# Product cards include real hrefs for copy/share/discovery while preserving modal UX.
...
# Initial deep link and browser history.
...
# Sitemap: only crawl the active storefront. Payment result pages are inactive and not canonical content.
